...
1. Added some keys for specifying what types of progress we want to return from certain scripts, still need to:
    - Add these keys to the SavorWorker and PhaseScriptVM
...
New shape to implement:
 ...
 - WorkerCoordinator replaces ParallelPhaseScriptRunner. One controller thread + N ProcessWorker threads; each thread owns a SavorWorker.exe child (one Dolphin per process).
...
#### Responsibilities:
 ...
 - Process control: spawn/kill SavorCoreSandbox/SavorWorker child processes; one process per worker thread (as you do now).
