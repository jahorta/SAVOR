...
namespace savor::inputtrace {

    struct BattleTurnInputTrace {
        uint32_t turn_number{ 0 };
        uint32_t vi_start{ 0 };
        uint32_t vi_end{ 0 };
        std::vector<uint32_t> vi_durations{};
        savor::ControllerInputSequence frames{};
    };
    ...
    inline bool append_turn_input_trace(std::string& blob, const BattleTurnInputTrace& trace) {
        if (!ensure_header(blob)) return false;
        if (trace.vi_durations.size() != trace.frames.size()) return false;
        ...
        write_u32(blob, trace.turn_number);
        write_u32(blob, trace.vi_start);
        write_u32(blob, trace.vi_end);
        write_u32(blob, static_cast<uint32_t>(trace.frames.size()));
        write_u32(blob, static_cast<uint32_t>(trace.vi_durations.size()));

        if (!trace.frames.empty()) {
            blob.append(reinterpret_cast<const char*>(trace.frames.data()), trace.frames.size() * sizeof(savor::GCInputFrame));
        }
        if (!trace.vi_durations.empty()) {
            blob.append(reinterpret_cast<const char*>(trace.vi_durations.data()), trace.vi_durations.size() * sizeof(uint32_t));
        }
    ...
    inline bool decode_turn_input_traces(const std::string& blob, std::vector<BattleTurnInputTrace>& out) {
        out.clear();
        ...
        for (uint32_t i = 0; i < seg_count; ++i) {
            ...
            const size_t frame_bytes = static_cast<size_t>(frame_count) * sizeof(savor::GCInputFrame);
    ...
    inline std::string turn_windows_csv(const std::vector<BattleTurnInputTrace>& traces) {
        std::ostringstream out;
        for (size_t i = 0; i < traces.size(); ++i) {
            if (i) out << ";";
            out << "t" << traces[i].turn_number << ":" << traces[i].vi_start << "-" << traces[i].vi_end;
        }
        return out.str();
    }

    inline std::string durations_csv(const std::vector<BattleTurnInputTrace>& traces) {
        std::ostringstream out;
        for (size_t i = 0; i < traces.size(); ++i) {
            if (i) out << ";";
            out << "t" << traces[i].turn_number << ":";
            for (size_t j = 0; j < traces[i].vi_durations.size(); ++j) {
                if (j) out << ",";
                out << traces[i].vi_durations[j];
            }
        }
        return out.str();
    }

    inline std::string render_text(const std::vector<BattleTurnInputTrace>& traces) {
        std::ostringstream out;
        for (const auto& c : traces) {
            ...
            for (size_t i = 0; i < c.frames.size(); ++i) {
                ...
                out << "  f" << i
                    << " vi_start=" << vi_cursor
                    << " vi_dur=" << vi_dur
                    << " : " << savor::DescribeFrameCompact(c.frames[i]) << "\n";
                vi_cursor += vi_dur;
            }
        }
        return out.str();
    ...
    inline savor::ControllerInputSequence flatten_controller_input_sequence(const std::vector<BattleTurnInputTrace>& traces) {
        savor::ControllerInputSequence out;
        size_t n = 0;
        for (const auto& c : traces) n += c.frames.size();
        out.reserve(n);
        for (const auto& c : traces) out.insert(out.end(), c.frames.begin(), c.frames.end());
        return out;
    }
}
