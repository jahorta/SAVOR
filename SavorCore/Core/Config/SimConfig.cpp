...
namespace savor {
    namespace {
        ...
        static inline std::filesystem::path NormalizePathForConfig(const std::filesystem::path& path) {
            std::error_code ec;
    ...
    namespace SimConfigIO {
        ...
        std::filesystem::path DefaultConfigPath() {
            ...
            return base / "SAVOR" / "simulator.ini";
#else
            // ~/.config/SAVOR/simulator.ini
            const char* home = std::getenv("HOME");
            std::filesystem::path base = home && *home ? std::filesystem::path(home)
                : std::filesystem::current_path();
            return base / ".config" / "SAVOR" / "simulator.ini";
#endif
        }
        ...
        bool Save(const SimConfig& cfg, const std::filesystem::path& path, std::string* error_out) {
            try {
                ...
                os << "# SAVOR simulator configuration\n"
                    "# Stores paths for your isolated User folder and a DolphinQt *portable* base.\n"
                    "\n[Paths]\n";
                os << "user_dir=" << ToUtf8(NormalizePathForConfig(cfg_to_save.user_dir)) << "\n";
                os << "dolphin_base_dir=" << ToUtf8(NormalizePathForConfig(cfg_to_save.dolphin_base_dir)) << "\n";
                os << "execution_db_path=" << ToUtf8(NormalizePathForConfig(cfg_to_save.execution_db_path)) << "\n";
                os << "state_db_path=" << ToUtf8(NormalizePathForConfig(cfg_to_save.state_db_path)) << "\n";
                os << "analysis_db_path=" << ToUtf8(NormalizePathForConfig(cfg_to_save.analysis_db_path)) << "\n";
                os << "authoring_db_path=" << ToUtf8(NormalizePathForConfig(cfg_to_save.authoring_db_path)) << "\n";
                os << "ui_read_db_path=" << ToUtf8(NormalizePathForConfig(cfg_to_save.ui_read_db_path)) << "\n";
                os << "archive_db_path=" << ToUtf8(NormalizePathForConfig(cfg_to_save.archive_db_path)) << "\n";
                os << "object_store_root=" << ToUtf8(NormalizePathForConfig(cfg_to_save.object_store_root)) << "\n";
                os << "archive_store_root=" << ToUtf8(NormalizePathForConfig(cfg_to_save.archive_store_root)) << "\n";
