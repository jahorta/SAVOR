...
std::vector<Byte> ContinueConfig(){StaticWriter w({'C','U','C','1'});w.U8(1);w.Bool(true);w.Bool(true);w.U8(0);w.U8(0);return w.Finish();}
...
ProgramValueId ContinueTo(Builder&b,ProgramFunction&f,BasicBlock&block,std::span<const SemanticPointReference> points,std::string selector,ProgramScopeId parent_scope={}){const auto scope=b.NewScope();(void)b.AddInstruction(f,block,InstructionOpcode::EnterScope,std::nullopt,{}, {},selector+"/scope",std::nullopt,scope);const auto point_set=Bytes(b,f,block,CanonicalRuntimeSchema::SemanticPointSet,EncodeSemanticPointSetV1(points),selector+"/semantic-points",scope);const auto no_input=Optional(b,f,block,CanonicalRuntimeSchema::OptionalInputExecutionBinding,std::nullopt,selector+"/no-input",scope);const auto no_movie=Optional(b,f,block,CanonicalRuntimeSchema::OptionalMoviePlaybackSession,std::nullopt,selector+"/no-movie",scope);const auto no_count=Optional(b,f,block,CanonicalRuntimeSchema::OptionalMovieInputCount,std::nullopt,selector+"/no-count",scope);const auto cc=Bytes(b,f,block,CanonicalRuntimeSchema::ContinueUntilStaticConfig,ContinueConfig(),selector+"/continue-config",scope);const std::array fields{point_set,no_input,no_movie,no_count,cc};const auto request=Construct(b,f,block,CanonicalActionInputType(CanonicalAction::ExecutionContinueUntil),fields,selector+"/continue-request",scope);const auto result=Await(b,f,block,CanonicalAction::ExecutionContinueUntil,request,selector+"/continue");(void)b.AddInstruction(f,block,InstructionOpcode::ExitScope,std::nullopt,{}, {},selector+"/release-scope",std::nullopt,scope);(void)parent_scope;return result;}
