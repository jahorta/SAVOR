...
bool BattleProjector::ProjectAll(std::string* error_out) {
    ...
    constexpr const char* kSql =
        ...
        "SELECT turn_job_id,exec_job_id,wave_id,job_state,fake_attacks_this_turn,fake_attacks_used_before,rng_seed,delta_vi,pred_passed,pred_total,battle_outcome,started_at_utc,ended_at_utc "
        "FROM ab_turn_job "
        "WHERE 1 "
        "ON CONFLICT(turn_job_id) DO UPDATE SET "
        "exec_job_id=excluded.exec_job_id,wave_id=excluded.wave_id,job_state=excluded.job_state,fake_attacks_this_turn=excluded.fake_attacks_this_turn,"
