...
QueuedStateDb::QueuedStateDb(
    ...
    : inner_(inner)
    , config_(config)
    , read_lane_(std::make_unique<savor::db::core::QueuedDbLane>("state-read", config_.read_capacity))
    , write_lane_(std::make_unique<savor::db::core::QueuedDbLane>("state-write", config_.write_capacity)) {
}
...
savor::db::core::QueuedDbTelemetrySnapshot QueuedStateDb::GetTelemetrySnapshot() const {
