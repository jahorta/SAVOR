...
WorkflowStepScheduleResult SeedProbeUniqueJobPersistenceAdapter::EncodeForQueueing(std::int64_t domain_ref_id) const {
    ...
    for (const auto& row : grid_rows) {
        ...
        savor::RandSeedProbeEntry entry{};
        ...
        if (row.source_family == "main" || row.source_family == "MAIN") {
            entry.family = savor::SeedFamily::Main;
        } else if (row.source_family == "cstick" || row.source_family == "CSTICK") {
            entry.family = savor::SeedFamily::CStick;
        } else {
            entry.family = savor::SeedFamily::Triggers;
        }
    ...
    auto planned = savor::PlanJCTComboSamples(
        grid,
...
std::optional<savor::PSJob> SeedProbeUniqueRuntimeInitAdapter::MaterializePsJob(
    std::int64_t job_id,
...
SeedProbeUniqueResultMapper::SeedProbeUniqueResultMapper(savor::db::IExecutionDb* execution_db, savor::db::IAnalysisDb* analysis_db)
    : execution_db_(execution_db)
    , analysis_db_(analysis_db) {
}

std::string SeedProbeUniqueResultMapper::BuildResultIniFromPrResult(std::int64_t /*job_id*/, const savor::PRResult& result) const {
    ResultsIni out{};
