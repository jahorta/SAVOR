...
-- Unreleased hard cut: remove the former bundle/check-use model, every Battle
-- reference to it, and every interim caller-owned predicate identity.
ALTER TABLE au_battle_plan_turn DROP COLUMN default_predicate_bundle_revision_id;
...
-- The predicate authoring model is unreleased. Rebuild all three authoring
-- families so no caller-authored identity or interim semantic hash survives.
...
CREATE TABLE au_predicate_authoring_request (
    ...
    creation_request_key TEXT NOT NULL CHECK(
        length(creation_request_key)=32 AND
        creation_request_key=lower(creation_request_key) AND
...
CREATE TABLE au_predicate_definition_v2 (
    ...
    stable_key TEXT NOT NULL UNIQUE CHECK(
        substr(stable_key,1,21)='predicate.definition/' AND
        length(stable_key)=53 AND
...
CREATE TABLE au_predicate_execution_binding (
    ...
    stable_key TEXT NOT NULL UNIQUE CHECK(
        substr(stable_key,1,18)='predicate.binding/' AND
        length(stable_key)=50 AND
...
CREATE TABLE au_predicate_execution_binding_revision (
    ...
    created_at_utc INTEGER NOT NULL,
    published_at_utc INTEGER NULL,
    FOREIGN KEY(predicate_execution_binding_id)
        REFERENCES au_predicate_execution_binding(predicate_execution_binding_id),
...
CREATE TABLE au_predicate_execution_binding_witness_source (
    ...
    source_kind TEXT NOT NULL CHECK(source_kind IN
        ('CONCRETE_VALUE','CURRENT_HOOK_QUERY','CURRENT_HOOK_RECEIPT',
         'PINNED_GUEST_MEMORY','BASELINE_OBSERVATION')),
    value_builtin_type INTEGER NULL,
    value_schema_canonical_id TEXT NULL,
    value_schema_revision INTEGER NULL,
    value_schema_sha256 TEXT NULL,
    ...
    literal_text TEXT NULL,
    literal_blob BLOB NULL,
    observation_source_kind TEXT NULL CHECK(observation_source_kind IS NULL OR
        observation_source_kind IN ('GUEST_ADDRESS','REGISTERED_QUERY','HOOK_RECEIPT','REGISTERED_REDUCER')),
    ...
    baseline_update_policy TEXT NULL CHECK(baseline_update_policy IS NULL OR
        baseline_update_policy IN ('FIRST','LATEST')),
    PRIMARY KEY(predicate_execution_binding_revision_id,witness_ordinal),
    FOREIGN KEY(predicate_execution_binding_revision_id)
        REFERENCES au_predicate_execution_binding_revision(predicate_execution_binding_revision_id)
        ON DELETE CASCADE
);
...
CREATE TABLE au_predicate_group (
    ...
    stable_key TEXT NOT NULL UNIQUE CHECK(
        substr(stable_key,1,16)='predicate.group/' AND
        length(stable_key)=48 AND
...
CREATE TABLE au_predicate_group_member (
    ...
    FOREIGN KEY(predicate_group_revision_id)
        REFERENCES au_predicate_group_revision(predicate_group_revision_id) ON DELETE CASCADE,
    FOREIGN KEY(predicate_execution_binding_revision_id)
        REFERENCES au_predicate_execution_binding_revision(predicate_execution_binding_revision_id),
    FOREIGN KEY(guard_execution_binding_revision_id)
        REFERENCES au_predicate_execution_binding_revision(predicate_execution_binding_revision_id)
);
...
CREATE TABLE au_predicate_group_member_hook (
    ...
    FOREIGN KEY(predicate_group_revision_id,member_ordinal)
        REFERENCES au_predicate_group_member(predicate_group_revision_id,member_ordinal)
        ON DELETE CASCADE
);

ALTER TABLE au_battle_plan_turn
    ADD COLUMN default_predicate_group_revision_id INTEGER NULL
    REFERENCES au_predicate_group_revision(predicate_group_revision_id);
...
WHEN (SELECT revision_state FROM au_predicate_execution_binding_revision
      WHERE predicate_execution_binding_revision_id=NEW.predicate_execution_binding_revision_id)='PUBLISHED'
BEGIN SELECT RAISE(ABORT,'published predicate execution binding revisions are immutable'); END;
...
WHEN (SELECT revision_state FROM au_predicate_execution_binding_revision
      WHERE predicate_execution_binding_revision_id=OLD.predicate_execution_binding_revision_id)='PUBLISHED'
BEGIN SELECT RAISE(ABORT,'published predicate execution binding revisions are immutable'); END;
...
WHEN (SELECT revision_state FROM au_predicate_group_revision
      WHERE predicate_group_revision_id=NEW.predicate_group_revision_id)='PUBLISHED'
BEGIN SELECT RAISE(ABORT,'published predicate group revisions are immutable'); END;
CREATE TRIGGER au_predicate_group_member_published_update_immutable
BEFORE UPDATE ON au_predicate_group_member
WHEN (SELECT revision_state FROM au_predicate_group_revision
      WHERE predicate_group_revision_id=OLD.predicate_group_revision_id)='PUBLISHED'
BEGIN SELECT RAISE(ABORT,'published predicate group revisions are immutable'); END;
