...
This stage proves that workflow orchestration works with real worker execution by wiring the new SavorDb interfaces into runner scheduling paths, while preserving a dual-path fallback.
...
7. Add runner integration path under `SavorCore/Runner/Parallel/SavorDb/` that adapts workflow materialization to worker scheduling primitives.
...
8. Add a new coordinator integration header in that folder that adapts or replaces `DBWorkerCoordinator.h` usage with SavorDb interfaces and workflow scheduling hooks.
...
## SavorTests Validation Plan (Required for Stage 3c)

1. **Item 1 (Stage 3b contract verification)** — *Integration schema contract test*: in `SavorTests`, open a migrated execution/UIRead test DB and assert required workflow tables/columns/indexes and enum-domain values exist before running orchestration tests.
...
8. **Item 8 (new coordinator integration header/path)** — *Compile-time + behavior test*: build SavorTests target with new coordinator integration selected and run behavior tests proving terminal callbacks and materialization callbacks are invoked in expected order.
...
12. **Item 12 (minimum relay/projector coverage)** — *Projector replay test*: feed workflow events through relay/projector in SavorTests, then replay same stream and assert `ui_workflow_*` rows are identical and checkpoints advance idempotently.
...
16. **Item 16 (data integrity checks)** — *Post-run integrity SQL tests*: after each scenario, run integrity queries from SavorTests asserting one `job_set` per materialized step, terminal closure completeness, and no dangling edges.
...
18. **Item 18 (go/no-go recommendation evidence)** — *Gate aggregation test/report*: in SavorTests, aggregate parity %, recovery pass/fail, integrity pass/fail, and performance results into a machine-readable decision artifact for promotion review.
