...
- Block-level captures are collated to `slice_io_pairs` grouped by slice, where each pair entry includes `function_id`, `input_fields`, and `output` for downstream replay in `SoaSimFileParsing`.
