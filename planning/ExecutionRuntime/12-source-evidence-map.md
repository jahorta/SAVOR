...
| IC-090 | SavorQt currently registers TAS, SeedProbe, Battle Context, Battle Single Turn, Battle End, and Navigation Context DB descriptors. | `SavorQt/SavorDbRuntime.cpp:270-325` | Implemented code |
...
| IC-105 | Dynamic workflow-step persistence and battle multi-wave behavior | `SavorTests/test_savordb_fixture_sqlite.cpp`, especially tests beginning at lines 695, 799, 865, 981, 1130, and 1348 in the snapshot | Implemented code |
| IC-106 | Real-worker/E2E phase surfaces | `SavorE2E/SeedProbeRealWorkerScenario.cpp`; `TasMovieRealWorkerScenario.cpp`; `BattleMacroProbeScenario.cpp`; `BattleSingleTurnScenario.cpp`; `BattleEndResultsScenario.cpp` | Implemented code |
...
| TD-005 | Programs are bounded; workflows own phase switching, waves, deduplication, recovery, and arbitrary-depth frontiers. | `06-workflows-frontiers-and-phase-composition.md` | Target decision |
...
| TD-007 | Survey, replay, collision search, cutscene acceleration, phase switching, and overworld DFS all use the same runtime and workflow boundaries. | `08-future-phase-reference-designs.md` | Target decision |
...
| TD-009 | Verification includes static verifier tests, deterministic fake-session traces, fault/unwind injection, differential parity, workflow recovery, and focused E2E. | `10-verification-and-acceptance.md` | Target decision |
| TD-010 | `ProgramKind` may remain semantic metadata only; built-in and authored programs compile to the same module; no `PK_UserScript`. | `11-decisions-risks-and-deferred-work.md` | Target decision |
...
| TD-012 | The first net-new phase using existing capabilities changes only its module, schemas, and workflow binding. | `README.md`; `02-target-execution-architecture.md`; `10-verification-and-acceptance.md` | Target decision |

## Interfaces and ownership affected

This evidence map tracks the replacement of the worker activation/result protocol, `ProgramRegistry`
switches, `PhaseScriptVM` ownership, `PSContext` as a public contract, the input-macro mini-runtime, and
the runtime-facing portions of `ProgramKindDescriptor`. It also tracks the retained ownership of durable
workflow topology and the new exclusive owners defined by documents 02 through 06.
...
| CF-003 | Router registry proposal centers stable wire kind, payload decoder, and program factory. | Exact module revision/hash, entrypoint, schemas, and dependency closure select execution; no arbitrary controller/program factory. | `ProgramKind` currently conflates identity, decoder, affinity, and workflow routing. |
...
| CF-006 | User-script plan uses generic `PSContext` for both request and result. | Entrypoints use typed input/output/emission/artifact schemas; locals and diagnostics are separate. | `PSContext` is global, flat, and directly includes current domain C++ types. |
| CF-007 | Current warm-worker affinity can avoid reactivation when kind/runtime/savestate strings match. | Affinity remains a locality optimization; correctness comes from exact module identity, explicit state policy, `StateEpoch`, and cleanup disposition. | Matching cache keys does not prove guest or host state cleanliness. |
...
| UR-002 | Concrete C++ signatures, module binary format, canonical hash algorithm, SQL DDL, and wire bytes | Logical contracts in documents 03-06 cannot change | `03`-`06`; document 11 | Unresolved research |
...
| UR-005 | Collision-oddity objective/scoring and refinement policy | Bounded program emits observations; workflow owns frontier/refinement | Same file `:88-92`; document 08 | Unresolved research |
| UR-006 | Overworld-specific movement, branching, goal, and pruning rules | Generic persisted frontier and bounded expansion contract is fixed | Document 08; document 11 | Unresolved research |
...
they must not assume byte equality when the new artifact intentionally adds provenance.
...
- final C++ signatures;
- SQL migrations or byte-level wire schema;
...
Those items become new evidence entries when they exist.
