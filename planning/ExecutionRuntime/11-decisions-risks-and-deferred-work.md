...
> is normative for stop routing, capture ownership, and progress. References
> below to generic route modes, phase-attached capture, live profile
> replacement, opaque capture-only progress, or free-form program progress are
> historical and must not guide implementation.
...
| D30 | Worker deployment mode is exactly `Headless`, `Visual`, or `VisualDebug`. Headless and Visual are scheduling-equivalent; VisualDebug is private to explicit replay/debug and excluded from `JobExecutionCoordinator`. | Worksets never request a mode. Interactive controls are valid only in VisualDebug, while ordinary Full Phase behavior is identical across the normal pool. |
...
| D32 | State artifacts use caller-declared new paths and immutable SHA-256, compatibility, and lineage receipts. A movie-paired savestate continuation carries exact embedded, hash-verified DTM history. | There is no ambient/latest artifact selection. Restore materializes the embedded history, requires any active read-only DTM identity to match, and verifies the prepared identity/mode plus any known cursor. External read-only import supplies no invented cursor: Dolphin restores it from the savestate and `MovieService` records the observed position. Recording file-artifact capture/import/restore is unsupported; recording rewind is limited to a same-session memory handle. |
| D33 | `InputArbiter` owns every pad publication through epoch-bound leases and supports both explicit interruption-borrow policies. | The engine sees only ephemeral execution bindings. Stable-neutral borrowing is decided from internal lease state; no phase supplies a neutral witness. Neutral apply/close are host-only when already neutral, while a transition from held to neutral must be observed before another held command when interaction semantics require a release gap. Movie playback holds an unsuspendable exclusive reservation. |
...
| D42 | Battle exit has three production Full Phases, `battle.completion`, `battle.record`, and the non-recording control `battle.replay`; Results Screen is a reusable interaction inside the eventual downstream phase. | Replay uses the exact BattleSet entry and branches on MovieService-owned restored state; no `battle.results_screen` program kind, workflow step, result aggregate, or standalone state exists. There is no monolithic `BattleEndResults`, Battle-specific exit SeedProbe, or Battle-to-Battle route. |
| D43 | Battle Completion and Battle Record share one adaptive completion interaction, accept either causal completion path, capture reward evidence, and end at the accepted fast/deferred field preseed. | Completion publishes `.bcmb` plus a movie-inactive planning state. Recording restores the movie-paired Battle-entry checkpoint as `Savestate` plus exact DTM continuation sidecar, adopts that restored playback without guest mutation, and publishes movie artifacts only after semantic equality and prefix/cursor verification. |
...
| D48 | The process seam constructs one uniform production `ProgramRuntime`/action host and publishes an exact immutable `WorkerRuntimeContractV1`. | There is no installed-program manifest, `Partial`/`CompleteExact` state, or program-kind advertisement. Exact Full Phase packages arrive only in worksets. |
...
| D50 | Each active item owns one bounded output transaction through finalization and terminal retention; the completion ledger owns only already-authoritative retained terminals. | Dolphin writes a native savestate file while paused and the runtime reads that completed file into immutable bytes. Host-only hashing/publication may follow execution unwind, but no later guest activity starts until finalization completes and the item terminal is retained. Raw state-buffer bytes remain private workset handles. |
...
| D52 | Production pool startup is progressive with at most two concurrent startups and one exact coordinator-computed runtime contract. | A matching Headless or Visual worker contributes capacity. A mismatched slot is terminated and quarantined without repeated restart; other matching workers continue. |
...
| D55 | A static-contract item credit covers one item from assignment through staged/resident/active/finalizing/terminal retention until exact acknowledgement. | Claim demand uses only unreserved credit and configured queue capacity. Warm affinity is a non-blocking tie-break and never excludes a cold worker. |
...
| D58 | The initial bounded production configuration is fixed in the runtime contract but coordinator queue capacity remains configured. | Defaults retain finite workset/item/finalizer/terminal limits and two concurrent startups. Tuning requires a new canonical runtime contract and cannot introduce worker capability classes or elapsed guest budgets. |
...
| D60 | The homogeneous hard cut uses WRMS v2 and workset wire v4. | WRMS v1 and workset v3 reject before session mutation. There is no decoder adapter, fallback negotiation, or mixed-version deployment. |
...
| D63 | Workset admission, guest initialization, readiness, and execution are distinct lifecycle states. | `SubmitWorkset` acknowledges host-only `Admitted`; initialization may run the guest; only the final paused commit creates authoritative execution evidence; `Running` begins automatically on the next actor turn. D63 supersedes D31's combined `BeginWorkset` wording. |
| D64 | Controller input is state-oriented: one lease owns either no active command (`Neutral`), a held non-neutral frame, a pending one-shot delivery, or a pending neutral transition. `apply_state` makes neutral an implicit release; `begin_delivery`/`complete_delivery` retain one typed one-shot receipt. | Guest synchronization is enforced by ephemeral execution bindings consumed by ContinueUntil/StepFrames. Stable neutral performs no publication or poll, bindings cannot become durable results, and no tape, vector, pulse, precomputed sequence, explicit poll action, neutral witness, direct pad setter, or PhaseScript input opcode remains. D64 supersedes all held-neutral wording in D33 and earlier source-evidence sections. |
| D65 | Every TAS Movie Full Phase workset contains exactly one item. | The rule is program-kind policy, independent of whether its baseline is a movie or savestate. Movie core startup/preparation and playback activation are separate operations so subscriptions exist before playback begins. |
| D66 | Battle Record branches an initialized read-only playback session into recording before guest advancement, while `InputArbiter` remains the sole controller publisher. | MovieService observes recorded inputs; it does not compete for controller ownership. ReplayMismatch publishes no partial movie artifacts, successful recordings validate automatically, and sterilization requires `Valid`. |
| D67 | Results Screen live/E2E/test-only Full Phase validation is deferred until a real downstream field, cutscene, or ship-runtime phase owns the handler. | Static construction, lowering, schema, reducer, and admission tests remain required. A temporary standalone phase is not a valid substitute for the downstream lifecycle. |
...
Rejected. The deprecated multi-turn BattleRunner does not become
`soa.battle.legacy_path`, and the old combined battle-end controller is not
recreated. Battle Completion and Battle Record are separate production
modules. Results Screen remains a reusable interaction inside the downstream
phase and has no standalone manifest/state workflow handoff.
...
| Adaptive interaction timing drifts | Input is applied after leaving the source, duplicate re-entry occurs, a semantic successor runs under the wrong binding, stable neutral publishes redundantly, or a held command is replaced before its neutral transition is observed | Characterize and test apply-before-departure, exact receipt suppression, successor point/PC/sequence/epoch matching, binding completion, canonical centered neutral encoding, and release-before-next-hold; forbid guest-step imports |
...
| Input borrowing corrupts held or neutral state | A handler overwrites an unsuspendable owner or resumes a parent against an unknown controller state | Make borrow policy explicit, preserve an untouched parent's state, require stable neutral from internal arbiter state when configured, and resume a parent in neutral after any borrower mutation; expose no phase-managed neutral witness |
...
- interaction unwind has one authority: input-lease cleanup restores a non-neutral backend once, closes
  an already-neutral lease without guest advancement, and releases nested subscriptions and the
  interaction-wide lease exactly once;
