# SOASim Architecture Research for Seed-Probed Battles and Multi-Mode Exploration
...
* Verify what the current SOASim repository already implements for TAS→probe→battle, seed dedupe, and turn-chaining.
...
## What the current SOASim repo already implements for your pathway

The current SOASim schema and program codecs already implement a substantial portion of the battle-side requirements, including the concept of “one job = one battle turn,” explicit dedupe rules keyed by RNG seed, and an explicit previous-turn link.
...
SOASim maintains an execution substrate built around:
...
This is effectively an embryonic orchestrator: it is a persisted “when condition holds, schedule action” artifact, which SOASim uses to chain phases.
...
For SOASim, that implies a stable split that matches ownership:
...
For SOASim specifically, the key benefit is that you can represent complex flows like:
...
In SOASim terms, the UI read DB should own:
