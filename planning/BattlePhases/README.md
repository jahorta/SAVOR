...
- Predicate definitions and versioned predicate bundles are first-class
  authored database objects. Referenced revisions are immutable.
- A Battle Plan turn may provide a default predicate bundle, while each created
  wave freezes the exact compatible bundle revision it will use. Separate waves
  for the same turn may intentionally use different bundles.
- A wave's predicate binding also freezes typed bundle parameters, its active
  check set, and aggregation configuration. Parameter values may change across
  waves without preparing a new program; structural hook, observation, type,
  topology, or active-check changes select another cached prepared variant.
- Every candidate in one wave/workset uses the same bundle revision so its
  predicate results remain comparable and the workset can reuse one prepared
  program variant.
- Each check explicitly combines one public semantic hook, an occurrence
  policy, typed observation and parameter bindings, a pure expression, evidence
  policy, reaction, and aggregation participation. Multiple hooks are modeled
  as multiple check uses rather than implicit record expansion.
...
- Predicate bundles are linked to versioned semantic hooks and verified once
  per exact phase, hook-contract, bundle, and capability-dependency key. They
  are not lowered or compiled separately for each candidate job.
- Coordination materializes a self-contained workset package containing the
  canonical resolved predicate bundle as shared input and the exact prepared
...
- The generalized workset model has explicit common inputs and per-job inputs.
  Common inputs contain phase-wide definitions, predicate bundles, bindings,
  and dependency identity once; item inputs contain the job's Battle Plan,
...
### 2026-08-07 - Automatic advancement and candidate availability

The legacy `auto_wave_trigger_enable` setting is retained. When disabled, a
later wave is created only after the user requests continuation from selected
`TurnInputs` jobs. When enabled, selection waits for the entire wave. Selection
is ephemeral coordination behavior, not a stored pruning judgment: all
...
outputs. The Battle-chain graph node publishes one `battle_set` output
...
### 2026-08-07 - Predicate bundle preparation, wave assignment, and worker isolation

Predicate definitions and immutable bundle revisions become first-class
authored database objects. Battle turns supply optional defaults, while every
wave freezes its exact selected bundle and may differ from other waves for the
same turn. A bundle is linked to stable phase semantic hooks and verified once
per exact content/dependency key, then reused by all compatible jobs. Before
dispatch, coordination resolves all database-backed authoring into a
self-contained workset package containing the shared canonical bundle and
prepared program definition. Workers never query SavorDb; they execute the
supplied package and return DB-independent typed results and staged artifacts
for coordination to persist.

### 2026-08-07 - Predicate authoring flexibility and wave bindings

Legacy flexibility is retained through explicit semantic hooks, typed
observations, coherent queries, checked address expressions, literals, wave
parameters, named baselines, pure expressions, occurrence policies, reactions,
and aggregation participation. Predicate bundles expose typed parameter schemas;
each wave freezes a binding containing its values, active checks, and aggregate
configuration. Value changes reuse the prepared bundle, while structural hook,
observation, type, topology, or active-check changes select another cached
variant. The authoring surface provides both a simple comparison editor and an
advanced typed expression editor without supporting arbitrary user scripts.
...
The shared predicate bundle, binding, prepared phase definition, and exact
dependencies are carried once per workset together with any genuinely shared
baseline. Each job carries its own Battle Plan as a per-job input, along with
