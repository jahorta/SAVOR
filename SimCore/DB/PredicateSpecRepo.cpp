...
namespace simcore {
    namespace db {
        ...
        static void bind_spec(sqlite3_stmt* st, const PredicateSpecRow& r) {
            ...
            // spec_version, required_bp, required_bp_multi, kind, width, cmp_op, flags,
            ...
            if (r.required_bp_multi.has_value()) sqlite3_bind_text(st, 3, r.required_bp_multi->c_str(), -1, SQLITE_TRANSIENT); else sqlite3_bind_null(st, 3);
            ...
            sqlite3_bind_text(st, 15, r.name.c_str(),        -1, SQLITE_TRANSIENT);
            sqlite3_bind_text(st, 16, r.description.c_str(), -1, SQLITE_TRANSIENT);
            sqlite3_bind_text(st, 17, r.fingerprint.c_str(), -1, SQLITE_TRANSIENT);
        }
        ...
        static inline PredicateSpecRow read_row(sqlite3_stmt* st) {
            ...
            // 0:id,1:spec_version,2:required_bp,3:required_bp_multi,4:kind,5:width,6:cmp_op,7:flags,
            // 8:lhs_addr,9:lhs_key,10:rhs_value,11:rhs_key,12:turn_mask,13:lhs_prog_id,14:rhs_prog_id,15:name,16:desc,17:fingerprint
            PredicateSpecRow r;
            r.id = sqlite3_column_int64(st, 0);
            r.spec_version = sqlite3_column_int(st, 1);
            r.required_bp = sqlite3_column_int(st, 2);
            if (sqlite3_column_type(st, 3) == SQLITE_NULL) r.required_bp_multi.reset();
            else r.required_bp_multi = std::string(reinterpret_cast<const char*>(sqlite3_column_text(st, 3)));
            ...
            r.name = reinterpret_cast<const char*>(sqlite3_column_text(st, 15));
            r.description = reinterpret_cast<const char*>(sqlite3_column_text(st, 16));
            r.fingerprint = reinterpret_cast<const char*>(sqlite3_column_text(st, 17));
            return r;
        }
        ...
        static inline DbResult<PredicateSpecRow> Impl_Get(DbEnv& env, int64_t id) {
            ...
            sqlite3_stmt* st{};
            int rc = sqlite3_prepare_v2(db,
                "SELECT id,spec_version,required_bp,required_bp_multi,kind,width,cmp_op,flags,lhs_addr,lhs_key,rhs_value,rhs_key,turn_mask,lhs_prog_id,rhs_prog_id,name,desc,fingerprint "
