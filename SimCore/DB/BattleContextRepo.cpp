...
namespace simcore {
    namespace db {
        ...
        static DbResult<int64_t> Impl_Insert(DbEnv& env, int64_t job_set_id, int64_t job_id, int64_t artifact_id, int64_t savestate_id, int64_t codec_version) {
        ...
        static DbResult<std::optional<BattleContextRow>> Impl_GetByJob(DbEnv& env, int64_t job_id) {
            ...
            sqlite3_stmt* st{};
            const char* sql =
                "SELECT context_id,job_set_id,job_id,artifact_id,savestate_id,codec_version,created_at FROM battle_contexts WHERE job_id=? LIMIT 1;";
            if (sqlite3_prepare_v2(db, sql, -1, &st, nullptr) != SQLITE_OK)
            ...
            row.savestate_id = sqlite3_column_int64(st, 4);
            row.codec_version = sqlite3_column_int64(st, 5);
            row.created_at = sqlite3_column_int64(st, 6);
            sqlite3_finalize(st);
        ...
        static DbResult<std::vector<BattleContextRow>> Impl_ListByJobSet(DbEnv& env, int64_t job_set_id) {
            ...
            sqlite3_stmt* st{};
            const char* sql =
                "SELECT context_id,job_set_id,job_id,artifact_id,savestate_id,codec_version,created_at FROM battle_contexts WHERE job_set_id=? ORDER BY context_id;";
            if (sqlite3_prepare_v2(db, sql, -1, &st, nullptr) != SQLITE_OK)
        ...
        std::future<DbResult<int64_t>> BattleContextRepo::InsertAsync(int64_t job_set_id, int64_t job_id, int64_t artifact_id, int64_t savestate_id, int64_t version, RetryPolicy rp) {
            return DBService::instance().submit_res<int64_t>(OpType::Write, Priority::Normal, rp, 
                [=](DbEnv& env) { return Impl_Insert(env, job_set_id, job_id, artifact_id, savestate_id, version); });
        }
        ...
        static DbResult<std::optional<BattleContextRow>> Impl_GetLatestBySavestateId(DbEnv& env, int64_t savestate_id) {
            sqlite3* db = env.handle();
            sqlite3_stmt* st{};
            const char* sql =
                "SELECT context_id,job_set_id,job_id,artifact_id,savestate_id,codec_version,created_at FROM battle_contexts "
                "WHERE savestate_id=? ORDER BY created_at DESC LIMIT 1;";
            if (sqlite3_prepare_v2(db, sql, -1, &st, nullptr) != SQLITE_OK)
                return DbResult<std::optional<simcore::db::BattleContextRow>>::Err({ map_sqlite_err(sqlite3_errcode(db)), sqlite3_errcode(db), sqlite3_errmsg(db) });
            sqlite3_bind_int64(st, 1, savestate_id);
            int rc = sqlite3_step(st);
            if (rc == SQLITE_DONE) { sqlite3_finalize(st); return DbResult<std::optional<simcore::db::BattleContextRow>>::Ok(std::nullopt); }
            simcore::db::BattleContextRow r{};
            r.context_id = sqlite3_column_int64(st, 0);
            r.job_set_id = sqlite3_column_int64(st, 1);
            r.job_id = sqlite3_column_int64(st, 2);
            r.artifact_id = sqlite3_column_int64(st, 3);
            r.savestate_id = sqlite3_column_int64(st, 4);
            r.codec_version = sqlite3_column_int64(st, 5);
            r.created_at = sqlite3_column_int64(st, 6);
            sqlite3_finalize(st);
            return DbResult<std::optional<simcore::db::BattleContextRow>>::Ok(std::optional<simcore::db::BattleContextRow>(std::move(r)));
        }
        std::future<DbResult<std::optional<BattleContextRow>>> BattleContextRepo::GetLatestBySavestateIdAsync(int64_t savestate_id, RetryPolicy rp) {
            return DBService::instance().submit_res<std::optional<BattleContextRow>>(OpType::Read, Priority::Normal, rp,
                [=](DbEnv& env) { return Impl_GetLatestBySavestateId(env, savestate_id); });
        }
    }
} // namespace
