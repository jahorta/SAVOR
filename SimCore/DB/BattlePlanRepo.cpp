...
namespace simcore::db {
    ...
    static DbResult<int64_t> impl_ensure(DbEnv& env, int64_t settings_id, const std::string& name, const std::string& fp, int32_t num_turns) {
        auto f = impl_find_by_fp(env, fp);
        ...
        if (sqlite3_prepare_v2(db,
            "INSERT INTO battle_plan(settings_id,name,fingerprint,num_turns,created_at) "
        ...
        sqlite3_bind_text(st, 3, fp.c_str(), -1, SQLITE_TRANSIENT);
        sqlite3_bind_int(st, 4, num_turns);
        int64_t id{};
        if (sqlite3_step(st) == SQLITE_ROW) id = sqlite3_column_int64(st, 0);
        sqlite3_finalize(st);
        if (!id) return DbResult<int64_t>::Err({DbErrorKind::IO, sqlite3_errcode(db), "insert failed" });
        return DbResult<int64_t>::Ok(id);
    }

    static DbResult<std::vector<BattlePlanRow>> impl_list(DbEnv& env, int64_t settings_id) {
        auto* db = env.handle();
        sqlite3_stmt* st{};
        if (sqlite3_prepare_v2(db,
            "SELECT plan_id,settings_id,name,fingerprint,num_turns,created_at "
            "FROM battle_plan WHERE settings_id=? ORDER BY plan_id;", -1, &st, nullptr) != SQLITE_OK)
            return DbResult<std::vector<BattlePlanRow>>::Err({ map_sqlite_err(sqlite3_errcode(db)), sqlite3_errcode(db), sqlite3_errmsg(db) });
        sqlite3_bind_int64(st, 1, settings_id);
        std::vector<BattlePlanRow> v;
        int rc;
        while ((rc = sqlite3_step(st)) == SQLITE_ROW) {
            BattlePlanRow r;
            r.plan_id = sqlite3_column_int64(st, 0);
            r.settings_id = sqlite3_column_int64(st, 1);
            if (sqlite3_column_type(st, 2) != SQLITE_NULL) r.name = (const char*)sqlite3_column_text(st, 2);
            if (sqlite3_column_type(st, 3) != SQLITE_NULL) r.fingerprint = (const char*)sqlite3_column_text(st, 3);
            r.num_turns = sqlite3_column_int(st, 4);
            r.created_at = sqlite3_column_int64(st, 5);
            v.push_back(std::move(r));
        }
        sqlite3_finalize(st);
        if (rc != SQLITE_DONE) return DbResult<std::vector<BattlePlanRow>>::Err({ map_sqlite_err(sqlite3_errcode(db)), sqlite3_errcode(db), sqlite3_errmsg(db) });
        return DbResult<std::vector<BattlePlanRow>>::Ok(std::move(v));
    }
    ...
    std::future<DbResult<int64_t>> BattlePlanRepo::EnsureAsync(int64_t settings_id, std::string name, std::string fingerprint, int32_t num_turns, RetryPolicy rp) {
        return DBService::instance().submit_res<int64_t>(OpType::Write, Priority::Normal, rp,
            [=](DbEnv& e) { return impl_ensure(e, settings_id, name, fingerprint, num_turns); });
    }
    std::future<DbResult<std::vector<BattlePlanRow>>> BattlePlanRepo::ListBySettingsAsync(int64_t settings_id, RetryPolicy rp) {
        return DBService::instance().submit_res<std::vector<BattlePlanRow>>(OpType::Read, Priority::Normal, rp,
