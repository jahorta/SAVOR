...
namespace simcore::progress {
	...
	using BattleProgressFxn = std::function<std::string(simcore::DolphinWrapper&)>;
	...
	static inline std::string get_combatant_name(simcore::DolphinWrapper& dw, uint32_t slot) {
		uint16_t id = 0xffffu;
	...
	static inline std::string describe_instruction(simcore::DolphinWrapper& dw, soa::InstructionSet inst) {
	...
	static auto BattleProgress = std::to_array<BattleProgressEntry>({
		...
		{
			0x80081d04,
			[](simcore::DolphinWrapper& dw)->std::string
			{
