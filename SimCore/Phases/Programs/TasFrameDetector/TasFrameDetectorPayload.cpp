...
namespace simcore::tasframedetector {
...
bool decode_payload(const std::vector<uint8_t>& in, PSContext& out_ctx)
{
    ...
    if (df.load(dtm_path)) {
        ...
        const size_t n = std::min(bytes.size(), simcore::tas::DtmFile::kMinHeader);
        std::vector<uint8_t> header(bytes.begin(), bytes.begin() + static_cast<std::ptrdiff_t>(n));
        if (header.size() >= simcore::tas::DtmFile::kOffRecordingStartTime + sizeof(uint64_t)) {
            std::fill(header.begin() + static_cast<std::ptrdiff_t>(simcore::tas::DtmFile::kOffRecordingStartTime),
                header.begin() + static_cast<std::ptrdiff_t>(simcore::tas::DtmFile::kOffRecordingStartTime + sizeof(uint64_t)),
                uint8_t(0));
        }
