...
class SimCoreDbArtifactService {
public:
    static bool StorageReady() {
        return soasimqt2::SimCoreDbRuntime::instance().isRunning()
            && !ObjectStoreRoot().empty();
    }

    static ServiceResult<simcore::db::UiReadPage<simcore::db::UiArtifactSummary>> ListArtifacts(
    ...
    static ServiceResult<simcore::db::UiArtifactSummary> ImportArtifact(const ArtifactImportRequest& request) {
        ...
        if (!std::filesystem::exists(request.source_path) || !std::filesystem::is_regular_file(request.source_path)) {
            return Invalid<simcore::db::UiArtifactSummary>("source path is not a readable file");
        }
        ...
        if (display_filename.empty()) {
            return Invalid<simcore::db::UiArtifactSummary>("filename is required");
        }
        ...
        try {
            ...
            return Failed<simcore::db::UiArtifactSummary>(ex.what());
        }
        if (sha.empty()) {
            return Failed<simcore::db::UiArtifactSummary>("failed computing artifact SHA-256");
        }

        std::error_code ec;
        const auto size = static_cast<std::uint64_t>(std::filesystem::file_size(request.source_path, ec));
        if (ec) {
            return Failed<simcore::db::UiArtifactSummary>("failed reading artifact size: " + ec.message());
        }

        const auto stored_path_result = CopyToObjectStore(request.source_path, sha, display_filename);
        if (!stored_path_result.ok) {
            return ServiceResult<simcore::db::UiArtifactSummary>::Err(std::move(stored_path_result.error));
        }

        const auto now = simcore::db::types::UtcNow();
        simcore::db::StoreArtifactCommand command{};
        command.sha256 = sha;
        ...
        command.causation_id = "SoaSimQt2";

        std::int64_t artifact_id = 0;
        std::string error;
        if (!state_db->StoreArtifact(command, &artifact_id, &error)) {
            return Failed<simcore::db::UiArtifactSummary>(error);
        }

        simcore::db::UiArtifactSummary summary{};
        ...
        return ServiceResult<simcore::db::UiArtifactSummary>::Ok(std::move(summary));
    }
...
private:
    static simcore::db::IStateDb* StateDb() {
        return soasimqt2::SimCoreDbRuntime::instance().stateDb();
    }
    ...
    static std::filesystem::path ObjectStoreRoot() {
        const auto root = soasimqt2::SimCoreDbRuntime::instance().root();
