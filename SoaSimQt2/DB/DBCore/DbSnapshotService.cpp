...
std::filesystem::path MakeRestoreCopyPath() {
    const auto stamp = std::chrono::steady_clock::now().time_since_epoch().count();
    return std::filesystem::temp_directory_path() / ("soasimqt2-restore-" + std::to_string(stamp) + ".soasnap");
}
...
bool CollectSnapshotFiles(
    ...
    if (!std::filesystem::exists(root, ec) || !std::filesystem::is_directory(root, ec)) {
        if (error_out != nullptr) {
            *error_out = "SimCoreDB root does not exist: " + root.string();
        }
        return false;
    }
    ...
    for (const auto& entry : std::filesystem::recursive_directory_iterator(root, ec)) {
        if (ec) {
            if (error_out != nullptr) {
                *error_out = "failed scanning SimCoreDB root: " + ec.message();
            }
            return false;
        }
...
DbSnapshotResult RestartRuntime(
    soasimqt2::SimCoreDbRuntime& runtime,
    const std::filesystem::path& root,
    ...
    if (result.ok) {
        return DbSnapshotResult::Err("snapshot operation completed, but failed restarting SimCoreDB: " + restart_error);
    }
    return DbSnapshotResult::Err(result.error + "; additionally failed restarting SimCoreDB: " + restart_error);
}
...
DbSnapshotResult DbSnapshotService::SaveSnapshot(
    ...
    if (root.empty()) {
        return DbSnapshotResult::Err("SimCoreDB root is empty");
    }
    ...
    do {
        ...
        if (!InsertMetadata(db, "format", "soasimqt2-simcoredb-snapshot-v1", &error)
...
DbSnapshotResult DbSnapshotService::LoadSnapshot(
    ...
    if (target_root.empty()) {
        return DbSnapshotResult::Err("target SimCoreDB root is empty");
    }
    ...
    do {
        ...
        if (ec) {
            result = DbSnapshotResult::Err("failed clearing target SimCoreDB root: " + ec.message());
            break;
        }
        fs::create_directories(target, ec);
        if (ec) {
            result = DbSnapshotResult::Err("failed creating target SimCoreDB root: " + ec.message());
            break;
        }
