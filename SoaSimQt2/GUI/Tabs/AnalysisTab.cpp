...
QFrame* createToolCard(
    soasimqt2::gui::WorkspacePageShell* shell,
    const soasimqt2::gui::UiEntityRef& entity,
    const QString& title,
    const QString& body,
...
void AnalysisTab::build()
{
    ...
    auto* db = soasimqt2::SimCoreDbRuntime::instance().uiReadDb();
    if (db != nullptr) {
        simcore::db::UiReadSeedProbeRunListQuery seedQuery{};
        seedQuery.limit = 20;
    ...
    const auto explorerGroups = soasimqt2::db::SimCoreDbExplorerRunService::ListGroups(explorerQuery);
    ...
    simcore::db::UiReadArtifactListQuery artifactQuery{};
    artifactQuery.limit = 25;
    const auto artifacts = soasimqt2::db::SimCoreDbArtifactService::ListArtifacts(artifactQuery);
