...
void CoordinatorController::updateValidationMessage()
{
    ...
    if (!soasimqt2::SimCoreDbRuntime::instance().isRunning()) {
        issues.append(QStringLiteral("SimCoreDB runtime is not running."));
    } else if (soasimqt2::SimCoreDbRuntime::instance().executionDb() == nullptr
        || soasimqt2::SimCoreDbRuntime::instance().stateDb() == nullptr
        || soasimqt2::SimCoreDbRuntime::instance().analysisDb() == nullptr
        || soasimqt2::SimCoreDbRuntime::instance().authoringDb() == nullptr) {
        issues.append(QStringLiteral("SimCoreDB services are incomplete."));
    }
    ...
    if (!fileExists(workerExePath())) {
        issues.append(QStringLiteral("SimCoreWorker.exe was not found next to SoaSimQt2."));
    }
...
{
    simcore::runner::parallel::simcoredb::DBWorkflowWorkerCoordinatorConfig cfg{};
...
QString CoordinatorController::visualReplayStateToText(
    simcore::runner::parallel::simcoredb::VisualReplayRuntimeState state) const
{
    using simcore::runner::parallel::simcoredb::VisualReplayRuntimeState;
    switch (state) {
...
{
    return QDir(QCoreApplication::applicationDirPath()).filePath(QStringLiteral("SimCoreWorker.exe"));
}
