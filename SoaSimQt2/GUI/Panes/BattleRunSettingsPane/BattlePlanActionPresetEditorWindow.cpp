...
QString targetKindLabel(simcore::db::BattlePlanTargetKind kind)
{
    ...
    case simcore::db::BattlePlanTargetKind::SameAsOtherPC: return QStringLiteral("Same As Other PC");
...
void BattlePlanActionPresetEditorWindow::createWidgets()
{
    ...
    macroCombo_->addItem(macroLabel(simcore::db::BattlePlanActionMacro::Attack), static_cast<int>(simcore::db::BattlePlanActionMacro::Attack));
    macroCombo_->addItem(macroLabel(simcore::db::BattlePlanActionMacro::Defend), static_cast<int>(simcore::db::BattlePlanActionMacro::Defend));
    macroCombo_->addItem(macroLabel(simcore::db::BattlePlanActionMacro::Focus), static_cast<int>(simcore::db::BattlePlanActionMacro::Focus));
    macroCombo_->addItem(macroLabel(simcore::db::BattlePlanActionMacro::FakeAttack), static_cast<int>(simcore::db::BattlePlanActionMacro::FakeAttack));
    macroCombo_->addItem(macroLabel(simcore::db::BattlePlanActionMacro::UseItem), static_cast<int>(simcore::db::BattlePlanActionMacro::UseItem));

    targetKindCombo_ = new QComboBox(panel);
    targetKindCombo_->addItem(targetKindLabel(simcore::db::BattlePlanTargetKind::SingleEnemy), static_cast<int>(simcore::db::BattlePlanTargetKind::SingleEnemy));
    targetKindCombo_->addItem(targetKindLabel(simcore::db::BattlePlanTargetKind::MultipleEnemies), static_cast<int>(simcore::db::BattlePlanTargetKind::MultipleEnemies));
    targetKindCombo_->addItem(targetKindLabel(simcore::db::BattlePlanTargetKind::AnyEnemy), static_cast<int>(simcore::db::BattlePlanTargetKind::AnyEnemy));
    targetKindCombo_->addItem(targetKindLabel(simcore::db::BattlePlanTargetKind::SameAsOtherPC), static_cast<int>(simcore::db::BattlePlanTargetKind::SameAsOtherPC));
...
void BattlePlanActionPresetEditorWindow::loadNew()
{
    ...
    targetKindCombo_->setCurrentIndex(static_cast<int>(simcore::db::BattlePlanTargetKind::AnyEnemy));
    targetSingleSlotSpin_->setValue(kMinTargetSingleSlot);
...
void BattlePlanActionPresetEditorWindow::applyLoadedSnapshot(const simcore::db::BattlePlanActionPresetSnapshot& snapshot)
{
...
void BattlePlanActionPresetEditorWindow::loadSnapshot(const simcore::db::BattlePlanActionPresetSnapshot& snapshot, bool duplicate)
{
...
{
    const auto macro = static_cast<simcore::db::BattlePlanActionMacro>(macroCombo_->currentData().toInt());
    const bool needsTarget = (macro == simcore::db::BattlePlanActionMacro::Attack || macro == simcore::db::BattlePlanActionMacro::UseItem);
    const bool needsItem = (macro == simcore::db::BattlePlanActionMacro::UseItem);
    const auto targetKind = static_cast<simcore::db::BattlePlanTargetKind>(targetKindCombo_->currentData().toInt());
    const bool legacyTargetExpr = targetExprLabel_->isVisible();

    targetKindCombo_->setEnabled(needsTarget && !legacyTargetExpr);
    targetSingleSlotSpin_->setEnabled(needsTarget && !legacyTargetExpr && targetKind == simcore::db::BattlePlanTargetKind::SingleEnemy);
    targetMaskEdit_->setEnabled(needsTarget && !legacyTargetExpr && targetKind == simcore::db::BattlePlanTargetKind::MultipleEnemies);
    targetSameAsActorSpin_->setEnabled(needsTarget && !legacyTargetExpr && targetKind == simcore::db::BattlePlanTargetKind::SameAsOtherPC);
...
bool BattlePlanActionPresetEditorWindow::buildDraft(soasimqt2::db::BattlePlanActionPresetDraft& draft, QString* errorText) const
{
    const auto macro = static_cast<simcore::db::BattlePlanActionMacro>(macroCombo_->currentData().toInt());
    const auto targetKind = static_cast<simcore::db::BattlePlanTargetKind>(targetKindCombo_->currentData().toInt());
    ...
    if (macro == simcore::db::BattlePlanActionMacro::Attack || macro == simcore::db::BattlePlanActionMacro::UseItem) {
        switch (targetKind) {
    ...
    if (macro == simcore::db::BattlePlanActionMacro::UseItem) {
        if (itemText.isEmpty()) {
...
bool BattlePlanActionPresetEditorWindow::canSaveCurrentInPlace(const soasimqt2::db::BattlePlanActionPresetDraft& draft) const
{
...
{
    soasimqt2::db::BattlePlanActionPresetDraft draft{};
    QString error;
    if (!buildDraft(draft, &error)) {
        postStatusMessage(error, StatusToast::Severity::Warn);
        return;
    }
    const auto result = soasimqt2::db::SimCoreDbAuthoringService::SaveBattlePlanActionPreset(draft);
    if (!result.ok) {
...
void BattlePlanActionPresetEditorWindow::saveCurrent()
{
    ...
    const auto result = soasimqt2::db::SimCoreDbAuthoringService::RenameBattlePlanActionPreset(*loadedPresetId_, draft.name);
    if (!result.ok) {
        postStatusMessage(QString::fromStdString(result.error.message), StatusToast::Severity::Error);
        return;
    }
