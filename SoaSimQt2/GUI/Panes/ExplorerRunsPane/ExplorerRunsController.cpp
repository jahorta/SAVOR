...
using soasimqt2::db::ExplorerRunGroupQuery;
using soasimqt2::db::SimCoreDbExplorerRunService;
...
void ExplorerRunsController::kickGroupsFetch()
{
    ...
    groupsWatcher_.setFuture(QtConcurrent::run([query]() {
        return SimCoreDbExplorerRunService::ListGroups(query);
    }));
    emitStateChanged();
}
...
void ExplorerRunsController::kickGroupDetailFetch(qint64 jobSetId)
{
    ...
    groupDetailWatcher_.setFuture(QtConcurrent::run([jobSetId]() {
        return SimCoreDbExplorerRunService::GetGroupDetail(jobSetId, 1000);
    }));
    emitStateChanged();
}
...
void ExplorerRunsController::kickJobDetailFetch(qint64 jobId, bool force)
{
    ...
    jobDetailWatcher_.setFuture(QtConcurrent::run([jobId]() {
        return SimCoreDbExplorerRunService::GetJobDetail(jobId);
    }));
    emitStateChanged();
}
...
QString ExplorerRunsController::describeError(const soasimqt2::db::ServiceError& error, const QString& prefix) const
{
