...
std::optional<model::NjObjectRecord> parseObjectRecord(const NjcmDecodeContext& ctx,
    ...
    if (!attachRaw.has_value() || !childRaw.has_value() || !siblingRaw.has_value()) {
        ctx.out->diagnostics.push_back("Object record truncated while reading pointer fields.");
        return std::nullopt;
    }
