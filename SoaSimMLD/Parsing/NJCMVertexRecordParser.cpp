...
void parseVertexRecords(const NjcmDecodeContext& ctx, const std::size_t vertexListOffset, model::NjAttachRecord& attach) {
    ...
    for (std::size_t i = 0; i < 4096 && cur + 4 <= ctx.decoded.size(); ++i) {
        ...
        if (step == 0 || cur + step > ctx.decoded.size()) {
            ctx.out->diagnostics.push_back("Vertex chunk step exceeded buffer; stopping attach vertex decode.");
            break;
        }
