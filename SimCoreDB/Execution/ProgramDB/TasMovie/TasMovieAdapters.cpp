...
std::filesystem::path WorkingRoot(const std::filesystem::path& configured) {
    ...
    return std::filesystem::temp_directory_path() / "simcoredb-tasmovie";
}
...
public:
    ...
    WorkflowStepScheduleResult EncodeForQueueing(std::int64_t domain_ref_id) const override {
        ...
        set_cmd.program_kind = static_cast<std::int32_t>(simcore::PK_TasMovie);
        set_cmd.purpose = "TasMovie";
        set_cmd.created_by = "tasmovie_adapter";
        set_cmd.created_at_utc = simcore::db::types::UtcNow().time_since_epoch().count();
        ...
        for (std::int64_t rtc = cfg.rtc_low; rtc <= cfg.rtc_high; ++rtc) {
            ...
            enqueue.program_kind = static_cast<std::int32_t>(simcore::PK_TasMovie);
...
public:
    ...
    std::optional<simcore::PSJob> MaterializePsJob(std::int64_t job_id, const RuntimeInitRequest&) const override {
        ...
        simcore::PSJob out{};
        if (!simcore::tasmovie::encode_payload(spec, out.payload)) {
            return std::nullopt;
        }
        return out;
    }
...
public:
    ...
    std::string BuildResultIniFromPrResult(std::int64_t, const simcore::PRResult& result) const override {
        ...
        result.ps.ctx.get(simcore::keys::core::RUN_MS, out.run_ms_used);
        result.ps.ctx.get(simcore::keys::core::VI_FIRST, out.vi_start);
        result.ps.ctx.get(simcore::keys::core::VI_LAST, out.vi_end);
        result.ps.ctx.get(simcore::keys::tas::SAVE_PATH, out.savestate_path);
...
ProgramKindDescriptor BuildTasMovieDescriptor(
    ...
    descriptor.program_kind = simcore::PK_TasMovie;
