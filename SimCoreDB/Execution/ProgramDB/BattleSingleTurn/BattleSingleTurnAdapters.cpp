...
struct ResultsIni {
    ...
    std::uint32_t battle_outcome = static_cast<std::uint32_t>(simcore::battle::Outcome::Unknown);
...
std::filesystem::path WorkingRoot(const std::filesystem::path& configured) {
    ...
    return std::filesystem::temp_directory_path() / "simcoredb-battle-single-turn";
}
...
const simcore::db::BattlePlanTurnSnapshot* FindTurn(
    const simcore::db::BattlePlanSnapshot& plan,
    int turn_index) {
...
soa::battle::actions::TurnPlan BuildTurnPlan(
    const simcore::db::BattlePlanTurnSnapshot& turn,
...
std::vector<soa::battle::actions::TurnPlanSpec> CompileConcreteTurnSpecs(
    ...
    struct PlannedAction {
        simcore::db::BattlePlanActionSnapshot source;
...
std::vector<simcore::pred::Spec> BuildPredicates(
    const simcore::db::PredicateSetSnapshot* predicate_set,
    simcore::db::IAuthoringDb* authoring_db) {
    std::vector<simcore::pred::Spec> out;
    ...
    for (const auto& pred : predicate_set->predicates) {
        ...
        if (spec.has_flag(simcore::pred::PredFlag::RhsIsKey)) {
            spec.rhs_key = to_addr_key(pred.rhs_value);
        }
        if (pred.abort_on_fail) {
            spec.set_flag(simcore::pred::PredFlag::AbortOnFail);
        }
...
simcore::GCInputFrame InitialFrameFromUniqueSeed(const simcore::db::SeedProbeUniqueSeedRow& seed) {
...
std::string FingerprintFor(const JobIni& job) {
    ...
    return "PK=" + std::to_string(simcore::PK_BattleSingleTurnRunner)
...
bool IsContinuationOutcome(simcore::battle::Outcome outcome) {
    return outcome == simcore::battle::Outcome::ReachedNextTurn
        || outcome == simcore::battle::Outcome::Victory;
}

bool IsVictory(simcore::battle::Outcome outcome) {
    return outcome == simcore::battle::Outcome::Victory;
}

bool IsBadMaterialize(const ResultsIni& results) {
    if (results.battle_outcome != static_cast<std::uint32_t>(simcore::battle::Outcome::PlanMaterializeFailure)) {
        return false;
    }
...
public:
    ...
    WorkflowStepScheduleResult EncodeForQueueing(std::int64_t domain_ref_id) const override {
        ...
        if (!execution_db_->CreateJobSet(
                {
                    .program_kind = static_cast<std::int32_t>(simcore::PK_BattleSingleTurnRunner),
                ...
                if (!analysis_db_->RecordBattleTurnJob(
                        {
                            ...
                            .job_state = simcore::db::BattleTurnJobState::Queued,
                            .started_at_utc = now,
                            .recorded_at_utc = now,
        ...
        (void)analysis_db_->UpdateBattleTurnWaveStatus(
            wave->wave_id,
            simcore::db::BattleTurnWaveStatus::Running,
            std::nullopt,
            nullptr);
...
public:
    ...
    RuntimeInitRequest BuildRuntimeInit(std::int64_t job_id) const override {
        ...
        request.derived_buffer_type = simcore::DBuf::DK_Battle;
    ...
    std::optional<simcore::PSJob> MaterializePsJob(std::int64_t job_id, const RuntimeInitRequest&) const override {
        ...
        std::optional<simcore::db::PredicateSetSnapshot> predicate_set;
...
public:
    ...
    std::string BuildResultIniFromPrResult(std::int64_t, const simcore::PRResult& result) const override {
        ...
        result.ps.ctx.get(simcore::keys::battle::BATTLE_OUTCOME, out.battle_outcome);
        result.ps.ctx.get(simcore::keys::battle::PLAN_MATERIALIZE_ERR, out.plan_materialize_err);
        std::uint32_t before = 0;
        std::uint32_t cur = 0;
        result.ps.ctx.get(simcore::keys::battle::FAKE_ATTACK_USED_BEFORE, before);
        result.ps.ctx.get(simcore::keys::battle::FAKE_ATTACK_COUNT_THIS_TURN, cur);
        out.fake_attacks_used = before + cur;
        result.ps.ctx.get(simcore::keys::core::PRED_PASSED, out.pred_passed);
        result.ps.ctx.get(simcore::keys::core::PRED_TOTAL, out.pred_total);
        result.ps.ctx.get(simcore::keys::core::PRED_ABORT_RUN, out.pred_abort_run);
        std::string turn_blob;
        result.ps.ctx.get(simcore::keys::battle::APPLIED_INPUTPLAN_TURN_BLOB, turn_blob);
        if (!turn_blob.empty()) {
    ...
    ResultMapPayload MapPrimaryResult(std::int64_t job_id, const std::string& result_ini) const override {
        ...
        simcore::db::RecordBattleTurnJobCommand update{};
        ...
        update.job_state = failed ? simcore::db::BattleTurnJobState::Failed : simcore::db::BattleTurnJobState::Succeeded;
        update.ended_at_utc = simcore::db::types::UtcNow();
        ...
        payload.event_lines.push_back("[battle-single-turn-result-diagnostics] job=" + std::to_string(job_id)
            ...
            + " dw_err=" + std::string(simcore::RunToBpOutcomeToString(parsed.dw_err))
            + "(" + std::to_string(parsed.dw_err) + ")"
            + " battle_outcome=" + simcore::battle::get_outcome_string(parsed_outcome)
...
struct Survivor {
    simcore::db::BattleTurnJobSnapshot job;
    simcore::db::BattleTurnWaveSnapshot wave;
};
...
public:
    ...
    WorkflowTransitionDecision EvaluateTransition(const WorkflowTransitionContext& context) const override {
        ...
        const auto now = simcore::db::types::UtcNow();
        (void)analysis_db_->UpdateBattleTurnWaveStatus(current_wave->wave_id, simcore::db::BattleTurnWaveStatus::Completed, now, nullptr);
        ...
        if (!analysis_db_->EnsureBattleSelectionPool(
                {
                    ...
                    .criterion_kind = simcore::db::BattleSelectionCriterionKind::BestFakeAttacksByRngSeed,
                    .created_at_utc = now,
        ...
        std::map<std::int64_t, simcore::db::BattleTurnWaveSnapshot> wave_by_id;
        for (const auto& wave : waves) {
            ...
            (void)analysis_db_->RecordBattleSelectionDecision(
                {
                    ...
                    .decision_kind = winner ? simcore::db::BattleSelectionDecisionKind::Winner : simcore::db::BattleSelectionDecisionKind::Duplicate,
        ...
        for (const auto& [rng, winner] : best_by_rng) {
            (void)rng;
            any_victory = any_victory || (winner.job.battle_outcome.has_value() && IsVictory(*winner.job.battle_outcome));
        }
        if (any_victory) {
...
ProgramKindDescriptor BuildBattleSingleTurnDescriptor(
    ...
    descriptor.program_kind = simcore::PK_BattleSingleTurnRunner;
