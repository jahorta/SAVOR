...
QString snapshotPhaseLabel(savor::db::DbSnapshotPhase phase)
{
    using Phase = savor::db::DbSnapshotPhase;
    switch (phase) {
...
QString formatSnapshotProgressMessage(const savor::db::DbSnapshotProgress& progress, const QString& outputPath)
{
...
void SettingsPage::createWidgets()
{
    ...
    const CollapsibleSection storageSection = createCollapsibleSection(
        "SETTINGS SECTION",
        "Database Storage",
        "Manage the active Savor database root, switch to an existing database, and save or load portable database snapshots with artifacts.");
    ...
    QLabel* switchDescription = new QLabel(
        "Point SavorQt at another existing database root without copying data. The selected directory must contain SavorDb.sqlite3.",
...
void SettingsPage::loadState()
{
    ...
    if (persistedRoot_.isEmpty()) {
        ...
        setStatus(StatusKind::Info, "Loaded the saved database root preference from SavorQt settings.");
    }
}

void SettingsPage::refreshActiveRoot()
{
    activeRoot_ = normalizePath(QString::fromStdString(savorqt::SavorDbRuntime::instance().root().string()));
...
void SettingsPage::handleMoveDatabaseClicked()
{
    ...
    const auto answer = QMessageBox::question(
        this,
        "Move Database",
        QStringLiteral("Move the active database to a new root?\n\nCurrent root:\n%1\n\nNew root:\n%2\n\nSavorQt will stop database services, copy the database and artifacts, switch to the new root, and then delete the previous root contents.")
    ...
    startStorageOperation(
        ...
        [target = targetRoot.toStdString()]() {
            std::string error;
            const bool ok = savorqt::SavorDbRuntime::instance().relocateRoot(target, true, &error);
...
void SettingsPage::handleUseExistingDatabaseClicked()
{
    ...
    const auto answer = QMessageBox::question(
        ...
        QStringLiteral("Switch SavorQt to another existing database root?\n\nCurrent root:\n%1\n\nExisting root:\n%2\n\nNo data will be copied.")
            .arg(activeRoot_, targetRoot),
        QMessageBox::Yes | QMessageBox::No,
        QMessageBox::No);
    if (answer != QMessageBox::Yes) {
    ...
    startStorageOperation(
        ...
        [target = targetRoot.toStdString()]() {
            std::string error;
            const bool ok = savorqt::SavorDbRuntime::instance().switchRoot(target, &error);
            return StorageResult{ ok, ok ? QString() : QString::fromStdString(error) };
        });
}
...
void SettingsPage::handleSaveSnapshotClicked()
{
    const QString defaultPath = QDir(activeRoot_.isEmpty() ? QDir::homePath() : activeRoot_)
        .filePath(QStringLiteral("SavorSnapshot.soasnap"));
    const QString snapshotPath = QFileDialog::getSaveFileName(
        this,
        QStringLiteral("Save Database Snapshot"),
        defaultPath,
        QStringLiteral("Savor Snapshot (*.soasnap);;All Files (*)"));
    ...
    startStorageOperation(
        ...
        [path = normalizedSnapshotPath.toStdString(), outputPath = normalizedSnapshotPath, self]() {
            const auto result = savor::db::DbSnapshotService::SaveSnapshot(path, [self, outputPath](const savor::db::DbSnapshotProgress& progress) {
                if (!self) {
                    return;
                }
...
void SettingsPage::handleLoadSnapshotClicked()
{
    ...
    const QString prompt = overwritingCurrentRoot
        ...
        : QStringLiteral("Load this snapshot into the selected target root and switch SavorQt to it?\n\nSnapshot:\n%1\n\nTarget root:\n%2\n\nAny existing database or artifact files in that root may be overwritten.");
    ...
    startStorageOperation(
        ...
        [snapshot = normalizedSnapshotPath.toStdString(), target = normalizedTargetRoot.toStdString()]() {
            const auto result = savor::db::DbSnapshotService::LoadSnapshot(snapshot, target, true);
            return StorageResult{ result.ok, result.ok ? QString() : QString::fromStdString(result.error) };
        });
}
