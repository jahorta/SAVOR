...
{
    const auto settings = savorqt::db::SavorDbAuthoringService::ListExplorerSettings();
    if (!settings.ok) {
        return false;
    }
...
void TasSpecEditorWindow::loadSnapshot(const savor::db::TasSpecSnapshot& snapshot, bool duplicate)
{
    setWindowTitle(duplicate
...
void BattleRunSpecEditorWindow::loadSnapshot(const savor::db::BattleRunSpecSnapshot& snapshot, bool duplicate)
{
    setWindowTitle(duplicate
...
void ExplorerSettingsEditorWindow::loadSnapshot(const savor::db::ExplorerSettingsSnapshot& snapshot, bool duplicate)
{
...
void ExplorerSettingsEditorWindow::saveSpec()
{
    ...
    savorqt::db::ExplorerSettingsDraft draft{};
...
void BattleChainSpecEditorWindow::refreshChoices()
{
    ...
    const auto battleRunSpecs = savorqt::db::SavorDbAuthoringService::ListBattleRunSpecs();
    ...
    const auto explorerSettings = savorqt::db::SavorDbAuthoringService::ListExplorerSettings();
...
void BattleChainSpecEditorWindow::saveSpec()
{
    ...
    savorqt::db::BattleChainSpecDraft draft{};
    draft.name = nameEdit_->text().trimmed().toStdString();
    draft.description = descriptionEdit_->toPlainText().trimmed().toStdString();
    draft.battle_run_spec_id = *battleRunSpecId;
    draft.explorer_settings_id = *explorerSettingsId;
    const auto result = savorqt::db::SavorDbAuthoringService::SaveBattleChainSpec(draft);
    if (!result.ok) {
        postStatusMessage(QString::fromStdString(result.error.message), StatusToast::Severity::Error);
        return;
    }
