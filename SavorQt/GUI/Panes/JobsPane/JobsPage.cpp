...
void JobsPage::refreshModel()
{
    ...
    for (const savor::db::UiJobSummary& job : state.page.items) {
...
{
    const auto& state = controller_->viewState();
    const savor::db::UiJobSummary* selected = nullptr;
    for (const savor::db::UiJobSummary& job : state.page.items) if (job.job_id == state.selectedJobId) { selected = &job; break; }
    if (!selected) {
    ...
    for (const savor::db::ExecutionJobEventRecord& event : state.detail.events) {
