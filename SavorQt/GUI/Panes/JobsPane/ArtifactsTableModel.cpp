...
void ArtifactsTableModel::setArtifacts(const std::vector<savor::db::UiJobArtifact>& artifacts)
{
    int targetRow = 0;
    while (targetRow < static_cast<int>(artifacts.size())) {
        ...
        auto existingIt = std::find_if(artifacts_.begin() + std::min(targetRow, static_cast<int>(artifacts_.size())), artifacts_.end(), [targetArtifactId](const savor::db::UiJobArtifact& artifact) {
...
bool ArtifactsTableModel::artifactAffectsDisplay(const savor::db::UiJobArtifact& lhs, const savor::db::UiJobArtifact& rhs)
{
