...
QString portListText(const std::vector<savor::db::execution::workflow::WorkflowPortDefinition>& ports)
{
    QStringList lines;
...
QString workflowGraphText(const savor::db::WorkflowGraphSnapshot& graph)
{
...
void WorkflowGraphEditorWindow::loadSnapshot(const savor::db::WorkflowGraphSnapshot& snapshot, bool duplicate)
{
...
void WorkflowGraphEditorWindow::setSelectedGraphHidden(bool hidden)
{
    ...
    const auto result = savorqt::db::SavorDbAuthoringService::SetWorkflowGraphHidden(graphId, hidden);
    if (!result.ok) {
        postStatusMessage(QString::fromStdString(result.error.message), StatusToast::Severity::Error);
        return;
    }
...
void WorkflowGraphEditorWindow::saveGraph()
{
    ...
    savorqt::db::WorkflowGraphDraft draft{};
    ...
    for (const auto& node : nodes_) {
        ...
        savor::db::SaveWorkflowGraphNodeCommand node_command{};
    ...
    for (const auto& binding : outputBindings_) {
        draft.edges.push_back(savor::db::SaveWorkflowGraphEdgeCommand{
...
void WorkflowGraphEditorWindow::refreshPreview()
{
    ...
    savor::db::execution::workflow::WorkflowCompositionSpec spec{};
    spec.nodes = nodes_;
    spec.output_bindings = outputBindings_;

    const auto result = savorqt::db::SavorDbWorkflowService::PreviewComposition(spec);
    if (!result.ok) {
