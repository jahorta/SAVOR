...
inline std::filesystem::path MakeTempPhase4Dir(const std::string& suffix) {
    const auto stamp = std::chrono::duration_cast<std::chrono::microseconds>(
        std::chrono::system_clock::now().time_since_epoch())
                           .count();
    const auto dir = std::filesystem::temp_directory_path() / ("savortests-phase4-" + suffix + "-" + std::to_string(stamp));
    std::filesystem::create_directories(dir);
    return dir;
}

inline savor::db::DbConfigPaths MakePhase4DbPaths(const std::filesystem::path& base_dir) {
...
inline bool OpenPhase4ExecutionDb(
    ...
    std::unique_ptr<savor::db::core::DBService>* service_out,
    savor::db::execution::workflow::SqliteExecutionDb** execution_db_out,
...
inline void CleanupPhase4Db(std::unique_ptr<savor::db::core::DBService>& service, const std::filesystem::path& temp_dir) {
    if (service) {
        service->Stop();
        service.reset();
    }
