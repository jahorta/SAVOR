...
protected:
    void SetUp() override {
        temp_root_ = std::filesystem::temp_directory_path() / std::filesystem::path("savordb_tests_XXXXXX");
        const auto unique_suffix = std::to_string(std::chrono::steady_clock::now().time_since_epoch().count());
        temp_root_.replace_filename(std::string("savordb_tests_") + unique_suffix);
        ASSERT_TRUE(std::filesystem::create_directories(temp_root_));

        const auto shared_db_path = temp_root_ / "savordb_test.sqlite";
        ...
        db_service_ = std::make_unique<savor::db::core::DBService>(
            config_paths,
    ...
    std::unique_ptr<savor::db::core::DBService> db_service_;
    sqlite3* db_ = nullptr;
};
