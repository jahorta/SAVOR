...
TEST(FrameStep, BootGameThenStepOnce)
{
    ...
    auto iso = env_opt("SAVOR_TEST_ISO");
    if (!iso) GTEST_SKIP() << "Set SAVOR_TEST_ISO";
...
TEST(FrameStep, LoadSavestateThenStepOnce)
{
    ...
    auto iso = env_opt("SAVOR_TEST_ISO");
    auto state = env_opt("SAVOR_TEST_STATE");
    if (!iso || !state) GTEST_SKIP() << "Set SAVOR_TEST_ISO and SAVOR_TEST_STATE";
